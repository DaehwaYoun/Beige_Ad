...
for day in days:
    for rep in reps:
        ...
        for line in f:
            ...
            if start_pos < Ribosome_shift_factor[0] : # 5'UTR
                result[SP][0] += 1
            elif stop_pos > Ribosome_shift_factor[1] : # 3'UTR
                result[SP][2] += 1
            elif start_pos >= Ribosome_shift_factor[0] and stop_pos <= Ribosome_shift_factor[1] : # CDS
                result[SP][1] += 1
        print(SP+' done')
            
result = pd.DataFrame(result)
result.to_csv(prefix+'CDS-UTR_map.tsv', index=False, sep='\t')    